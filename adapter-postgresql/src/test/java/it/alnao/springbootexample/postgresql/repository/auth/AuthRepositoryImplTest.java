...
class AuthRepositoryImplTest {
    ...
    @Mock UserProviderJpaRepository providerJpaRepository;
    @Mock RefreshTokenJpaRepository tokenJpaRepository;

    @InjectMocks UserProviderRepositoryImpl providerRepository;
    @InjectMocks RefreshTokenRepositoryImpl tokenRepository;

    private UserProviderEntity providerEntity;
    ...
    void setup() {
        ...
        provider.setId("p-1");
        providerEntity = UserProviderEntity.fromDomain(provider);

        RefreshToken token = new RefreshToken("token-1", "u-1", LocalDateTime.now().plusDays(1));
        token.setId("t-1");
        tokenEntity = RefreshTokenEntity.fromDomain(token);
    }
    ...
    void provider_saveGeneratesAnIdWhenMissing() {
        ...
        when(providerJpaRepository.save(any(UserProviderEntity.class))).thenReturn(providerEntity);
    ...
    @Test
    void provider_findByUserIdDelegatesToTheDerivedQuery() {
    ...
    void token_saveGeneratesAnIdWhenMissing() {
        ...
        when(tokenJpaRepository.save(any(RefreshTokenEntity.class))).thenReturn(tokenEntity);
